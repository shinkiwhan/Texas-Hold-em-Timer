...
- **새틀라이트 모드**: 블라인드 레벨은 10분으로 고정한다. 앤티는 사용하지 않으며, 첫 상승 구간 이후 빅블라인드가 이전 레벨보다 주로 약 20~50%씩 오르는 계산하기 편한 구조를 사용한다. 블라인드 5레벨마다 5분 휴식을 넣는다.
...
이 구조는 사용자가 제시한 숫자 계열을 바탕으로, 공식 WSOP 대회 및 새틀라이트 구조에서 반복되는 `200, 400, 600, 800, 1,000, 1,200, 1,600, 2,000, 3,000, 4,000…` 빅블라인드 흐름을 적용한 것이다. 첫 `200 → 400` 구간은 100% 상승하지만 이후에는 대부분 약 20~50% 범위를 유지한다. 상승폭이 약 6~13%에 그치는 `16,000/32,000`, `18,000/36,000` 단계는 제외하고 `15,000/30,000 → 20,000/40,000`으로 이어지게 한다.

참고한 공식 자료:

- [WSOP 2025 공식 스트럭처](https://www.wsop.com/pdfs/structuresheets/structure_5771_24730.pdf)
- [WSOP Circuit Mega Satellite 공식 스트럭처](https://www.wsop.com/pdfs/structuresheets/structure_1076.pdf)
- [WSOP Step 1 Satellite 공식 스트럭처](https://www.wsop.com/pdfs/structuresheets/structure_1559_17405.pdf)

포커 TDA 규칙은 경기 운영 규칙을 제공하지만 단일 블라인드표를 국제 표준으로 지정하지 않는다. 따라서 실행 중 계산식으로 블라인드를 만들지 않고, 여러 공식 대회 구조에서 공통적으로 쓰이는 숫자 흐름을 이 앱에 맞게 고정하여 중앙 화면과 참가자 화면이 항상 동일한 값을 표시하도록 한다.
...
- 첫 상승 구간을 제외한 새틀라이트 빅블라인드 상승률이 대부분 20~50% 범위인지
